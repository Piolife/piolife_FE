...
const NearbyMedLab = () => {
  ...
  const { data, loading, error } = useFetchData<any>(
    location
      ? `${API_URL}/api/v12/users/nearby-specialized?lat=${location.latitude}&lng=${location.longitude}&radius=50&role=medical_lab_services`
      : "",
    { token }
  );
  ...
  return (
    ...
    >
      ...
      <View className="py-[8px] px-[4%] gap-[16px] mt-12">
        ...
        <Text className="text-[#272757] text-[18px] ">Recomended Med Lab</Text>
        <CustomFlatList
          data={data || []}
